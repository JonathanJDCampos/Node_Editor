...
Application::Application(const char* name, int argc, char** argv) : m_Name(name), m_Platform(CreatePlatform(*this)), m_Renderer(CreateRenderer())
{
...
void Application::RecreateFontAtlas()
{
    ...
    m_DefaultFont = io.Fonts->AddFontFromFileTTF("../external/imgui-node-editor/examples/data/Play-Regular.ttf", 18.0f, &config);
    m_HeaderFont = io.Fonts->AddFontFromFileTTF("../external/imgui-node-editor/examples/data/Cuprum-Bold.ttf", 20.0f, &config);
...
void Application::Frame()
{
    ...
    if (m_Platform->HasWindowScaleChanged()) m_Platform->AcknowledgeWindowScaleChanged();
...
{
    return ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar
